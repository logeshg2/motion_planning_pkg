...
"""
RRT Connect implementation for robotic arms, currently the planner accepts n-vector dim start and goal configurations.
...
class RRTConnectPlanner:
    ...
    def plan(self, start_config: list, goal_config: list):
        ...
        self.tree1.append(Node(start_config))
        self.tree2.append(Node(goal_config))
        ...
        for itr in range(self.maxIterations):
            ...
            near_node = nearest(self.tree1, rand_node)
            ...
            # RRT-Connect additional steps
            goal_reached = Connect(
                self.tree2, 
                new_node, 
                self.model, 
                ...
                self.goal_threshold,
                self.steer_dist,
                self.jointLowerLimits,
                self.jointUpperLimits
            )

            if (not goal_reached):
                # tree flip flag
                self.treeFlip = 2 if (self.treeFlip == 1) else 1
                # swap trees
        ...
        # get path from trees
        # based on `self.treeFlip` flag construct path from start to goal config
        # NOTE: Tree 1 and Tree 2 are not connected
        path_tree1 = getPathNodesFromTree(self.tree1)
        path_tree2 = getPathNodesFromTree(self.tree2)
        if (self.treeFlip == 1):
            path_tree2.reverse()
            path_tree1.extend(path_tree2)
            path_nodes = path_tree1
        else:
            path_tree1.reverse()
            path_tree2.extend(path_tree1)
            path_nodes = path_tree2
        
        # perform shortcutting
        if (self.do_shortcutting):
            path_nodes = shortcut(path_nodes, self.model, self.collision_model, num_itr=80, verbose=self.verbose)
