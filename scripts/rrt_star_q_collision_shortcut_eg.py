...
"""
Example script for rrt star implementation for 6d configuration vector + with shortcut path (smooth out jerky motion).
"""
...
seed_num = 42
pinocchio.seed(seed_num)     # int(time.time())
...
goal_q = np.array([1.5, 0.7, 0.3, 0.0, -1.5, 0.0])
...
# path shortcut (NOTE: This removes jerky movement + unwanted movements)
...
# discretize the path
discretized_path = discretize_joint_position(shortcut_path)
input("Enter to continue simulation:")
...
# imageio.mimsave('output_animation.mp4', frames, fps=30)
###